...
// Request config with the custom, non-standard flag this app adds (see
// skipChannel usage below and in apiRoutes.getZone).
...
const getStoredLanguage = async (): Promise<any> => {
  const state = store.getState();
  return state?.Language?.selectedLanguage;
};

const getStoredChannel = async (): Promise<string | undefined> => {
  const state = store.getState();
...
export const buildApiHeaders = ({
  includeChannel = true,
}: { includeChannel?: boolean } = {}): Record<string, string> => {
  const state = store.getState();
...
const __recentCalls = new Map<string, number>(); // key -> last timestamp
const __DUP_WINDOW_MS = 1500;

const __callerComponent = (): string => {
...
const __logApiCall = (config: AppRequestConfig): void => {
  ...
  if (isDup) {
    console.warn(
      `🔁 DUPLICATE API (within ${Math.round(now - (prev as number))}ms) →`,
      config.url,
    );
  }
  console.table([row]);
