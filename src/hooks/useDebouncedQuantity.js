 ...
 *
 * @param {object}   opts
 * @param {number}   opts.serverQty   - authoritative qty from the cart store
 * @param {(qty:number, ctx:{reqId:number,isStale:()=>boolean})=>void|Promise} opts.onCommit
 ...
 * @param {(units:number, ctx:{reqId:number,isStale:()=>boolean})=>void|Promise} opts.onRemove
 *   - drop `units` from the line via the remove endpoint (used on decrease). Same ctx contract.
 * @param {number}   [opts.max]       - clamp ceiling (allowed/stock); optional
 * @param {()=>void} [opts.onMax]     - called when a +click is blocked by max
