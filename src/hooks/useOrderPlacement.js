...
export default function useOrderPlacement() {
  ...
  const statusUrl = (method, orderId) =>
    zoneHref(
  ...
  const fetchLatestOrderId = async () => {
    try {
      const res = await api.getOrders({ limit: 1, offset: 0, type: "", orderType: "" });
  ...
  const handleRazorpayCancel = async (orderId) => {
    await api.deleteOrder({ orderId });
  };

  const handleRazorpayPayment = async (orderId, transactionId, amount, method, capitalized, currency) => {
    ...
    const options = {
      ...
      // fall back to the setting value, then to "INR" as last resort.
      currency: currency || setting?.currency_code || "INR",
      name: user?.name,
  ...
  const handlePaystackPayment = async (orderId, amount, method, capitalized) => {
    try {
  ...
  const runTransaction = async (orderId, method, payable) => {
   ...
   * @param {object}  p
   * @param {string}  p.method       "COD" | "wallet" | gateway label
   * @param {number}  p.payable      amount to charge at the gateway
   * @param {number}  p.subTotal
   * @param {number}  p.deliveryCharge
   * @param {number}  p.finalTotal
   * @param {boolean} p.walletUsed
   * @param {number}  p.walletBalance
   * @param {number|string} p.addressId
   * @param {number}  p.promocodeId
   * @param {string}  p.orderType     default "doorstep"
   * @param {()=>void} p.onCreated    called after the order is created (e.g. close drawer)
   * @returns {Promise<boolean>} true when the order was created
   */
