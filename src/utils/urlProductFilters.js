...
// shared between the client hook (useUrlProductFilters) and getServerSideProps
// (src/pages/products/index.jsx) so server and client can never drift.
...
export function parseFilterFromQuery(query = {}) {
...
export function hasNoFilterParams(query = {}) {
...
// through unmodified.
export function serializeFilterPatch(currentQuery = {}, patch = {}) {
...
export function clearFilterQuery(currentQuery = {}, { preserveCategory = false } = {}) {
  const next = { ...currentQuery };
...
export function buildFilterApiParams(filter, { search } = {}) {
  const params = {
