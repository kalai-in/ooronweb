...
// provider.
const mapGoogleComponents = (components = []) => {
  const get = (type) =>
...
const backendReverse = async (lat, lng) => {
  const res = await getGeocoding({ latitude: lat, longitude: lng });
...
const normalizeOsmReverse = async (lat, lng) => {
  const r = await osmReverse(lat, lng);
  if (!r) return null;
 ...
 * @param {number} lat
 * @param {number} lng
 * @param {"google"|"osm"} provider
 */
export const reverseGeocode = async (lat, lng, provider) => {
 ...
 * @param {"google"|"osm"} provider
 */
export const forwardGeocode = (query, provider) => {
  if (provider === MAP_PROVIDERS.GOOGLE && hasGoogleGeocoder()) {
