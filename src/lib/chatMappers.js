...
const KNOWN_KINDS = ["image", "audio", "video", "file"];
const kindFromType = (t) => (KNOWN_KINDS.includes(t) ? t : null);
...
export const mapMessage = (m) => {
  ...
  const attachments = m.attachment_url
    ? [
        {
...
export const mapMessages = (list) => (list || []).map(mapMessage).filter(Boolean);
