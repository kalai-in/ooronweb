...
export function formatCustomDate(dateString?: string | null) {
  if (!dateString) return;
  return formatDateTime(sanitizeDate(dateString));
}

export const formatOnlyDate = (dateString?: string | null) => {
  if (!dateString) return;
  return formatDate(sanitizeDate(dateString), undefined);
};
