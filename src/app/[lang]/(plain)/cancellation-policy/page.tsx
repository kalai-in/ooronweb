...
const loadCancellationData = cache(async (lang: string) => {
  const { zoneId, notFound: zoneNotFound } = await resolveZoneForSeo(undefined);
  if (zoneNotFound) return { notFound: true as const };
  // Language codes + default from the API for hreflang — fetched independently
  // of the SEO row so canonical/hreflang still work even if the SEO API call
  // itself fails.
  const { codes, defaultCode: langDefault } = await getLanguagesCached();
  const seo = await fetchSeoRow("Cancellation policy", lang, zoneId);
...
// localized variants aren't treated as duplicates.
...
export async function generateMetadata({
  ...
  const result = await loadCancellationData(lang);
...
export default async function CancellationPolicyRoute({
  params,
}: {
  params: Params;
}) {
