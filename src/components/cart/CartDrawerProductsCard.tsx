...
// component type from its JSDoc param comments. Cast to `any` at the import
// boundary — see the identical note in CartProductCard.tsx.
...
interface CartDrawerProductsCardProps {
  /** cart row shape varies by API response — no shared exported type, kept `any`
   * like the rest of this migration's cart rows. */
  product: any;
  /** Current drawer row list — read in refreshGuestCart to merge a guest
   * qty/remove update onto the existing rich row instead of replacing it with
   * the bare redux guestCart shape. */
...
const CartDrawerProductsCard = ({
  ...
  currency,
  decimals,
}: CartDrawerProductsCardProps) => {
  const dispatch = useDispatch();
  const { guard: guardClosed, closedProps } = useStoreClosed();

  // Prefer currency/decimals passed down from the parent (already computed via
  // useCurrency); fall back to useCurrency() directly otherwise.
  ...
  // Guest cart is local-only (pure redux, no network) — same as CartProductCard's
  // cart-page handlers. This used to re-fetch getGuestCart on EVERY +/-/remove
  // click (no debounce), which meant a burst of clicks (or a fast repeated
  // remove) queued a pending network call per click. Local recompute matches the
  // established guest-cart pattern and drops the network call entirely.
  //
  // `guestCartItems` are bare redux rows (product_id/product_variant_id/qty/
  // productPrice only — see addtoGuestCart callers). The drawer's row list needs
  // the RICH fields (image_url, name, price, discounted_price, ...) that only
  // exist on the already-rendered rows, so merge qty updates onto the existing
  // row instead of replacing the list with the bare redux shape (which produced
  // blank cards + duplicate/undefined keys since bare rows have no `id`).
  const recalculateGuestTotal = (products?: any[]) => {
  ...
  const refreshGuestCart = (guestCartItems?: any[]) => {
    dispatch(addtoGuestCart({ data: guestCartItems ?? [] }));
    // Merge computed from the `cartProductsData` PROP (not a setState updater —
    // updaters must stay pure/side-effect-free; dispatching from inside one is an
    // unsafe cross-component render-phase update and React flags it).
    const prevRows = cartProductsData ?? [];
    const next = (guestCartItems ?? [])
      .map((guestRow: any) => {
        const priorRow = prevRows.find(
          (row: any) => row?.product_variant_id == guestRow?.product_variant_id,
        );
        return { ...priorRow, ...guestRow };
      })
      // A guest row with no matching rich row (shouldn't normally happen — rows
      // are added elsewhere with full product data first) can't render a usable
      // card; drop it rather than show a blank one.
      .filter((row: any) => row?.product_id != null);
    setCartProductsData(next);
    recalculateGuestTotal(next);
  };
  ...
  const syncFromServerCart = (response: any) => {
    ...
    setCartProductsData((prev) => {
      ...
      const next = serverItems.map((serverItem: any) => {
        const priorRow = prevRows.find(
          (row: any) => row?.product_variant_id == serverItem?.variant_id,
        );
        return {
          ...priorRow,
          ...serverItem,
  ...
  // Guards handleRemoveWholeLine against a second click while its request is
  // still in flight. Without this, mashing the X (or double-clicking) fired one
  // removeFromCart PER CLICK, all carrying the SAME captured lineQty (the row's
  // qty hadn't updated yet) — the backend saw repeated "remove all N units" calls
  // for a line that only had N, and whichever response landed LAST (not
  // necessarily the last one sent) won via syncFromServerCart, sometimes with an
  // inconsistent/empty cart payload → drawer flashed "Cart Is Empty" client-side
  // even though the server's actual last-committed state still had the items
  // (a refresh then showed them again, since nothing was ever really lost).
  ...
  const handleGuestIncrease = async () => {
    ...
    const productQty = productQuantity?.find(
      (prdct: any) => prdct?.product_id == product?.product_id,
    )?.qty;
    if (
  ...
  const serverAddedQty = cart?.cartProducts?.find(
    (prdct: any) => prdct?.product_variant_id == product?.product_variant_id,
  )?.qty;
  const guestAddedQty = cart?.guestCart?.find(
    (prdct: any) => prdct?.product_variant_id == product?.product_variant_id,
  )?.qty;

  // Upper bound for the stepper: MIN(remaining store-wide cart capacity,
  // per-product allowed quantity, available stock).
  ...
  // Same cap, computed against the GUEST cart — needed for the guest atMaxQty
  // below (server's own maxQty is computed above, against cartProducts).
  ...
  // spam past it even before the debounced commit lands. MIN(store-wide cap,
  // per-product cap, stock) — not just the per-product cap.
  const atMaxQty =
    Number(addedQuantity) >= (cart?.isGuest ? guestMaxQty : maxQty);
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[84px] w-[84px] shrink-0 overflow-hidden rounded-lg bg-gray-50 dark:bg-zinc-800">
        {/* ImageWithPlaceholder.tsx (out of scope for this migration batch, owned by
            a concurrent agent) doesn't type a `fill` prop, but this call already
            passed it pre-TS as a harmless extra prop — cast to preserve exactly. */}
