...
const CartProductCard = ({
  ...
  // Prefer currency passed from the /cart response; then the per-channel currency
  // cached in redux (set on every cart fetch); then settings. Settings no longer
  // carry a currency, so without the redux fallback prices lose their symbol when
  // the page renders from redux (no fresh currency prop).
  const cur = currency || cart?.currency || setting?.currency || "";
  const dec = decimals ?? cart?.decimal_point ?? setting?.decimal_point ?? 0;
  ...
  useEffect(() => {
    ...
    const productQty = productQuantity?.find(
      (prdct) => prdct?.product_variant_id == variantId,
    )?.qty;
    const finalPrice =
  ...
  const handleGuestCartRemove = () => {
    const remainItems = cart?.guestCart?.filter(
      (cartProduct) => cartProduct?.product_variant_id !== variantId,
    );
  ...
  const handleServerRemoveOptimistic = async () => {
    setCartProductsData((prev) =>
      (prev ?? []).filter(
        (row) => (row?.variant_id ?? row?.product_variant_id) != variantId,
      ),
    );
  ...
  // Upper bound: smaller of stock (when limited) and the per-product allowed qty.
