...
const Cart = () => {
  ...
  // Currency symbol + decimal precision. Settings no longer carry a currency, so the
  // symbol comes from the /cart response; when the page renders from the redux cart
  // (no fresh cartData), fall back to the per-channel currency cached in redux
  // (cart.currency, set via setCartCurrency on every cart fetch) — otherwise prices
  // render with no symbol (bare "150").
  const currency =
    cartData?.currency || cart?.currency || setting?.currency || "";
  // Decimal precision, like currency, rides in the /cart response — settings carry
  // neither anymore. When the page renders from redux (no fresh cartData), fall back
  // to the decimal_point on the redux cart response so prices keep their fraction
  // (150.00, not 150).
  const decimals =
