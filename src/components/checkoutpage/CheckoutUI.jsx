...
const CheckoutItemRow = ({ product, money, onCommitQty, onRemoveUnits }) => {
  ...
  // At the cap (total_allowed_quantity, or stock when limited) → grey out "+",
  // matching CartProductCard / HomeVerticleProductCard. The hook already refuses
  // to go past `maxQty`; this stops the button looking clickable when it can only
  // ever fire the max-limit toast. maxQty folds in the stock ceiling too, which
  // hasReachedAllowedQty alone doesn't cover.
  const atMaxQty =
    hasReachedAllowedQty(qty, product?.total_allowed_quantity) ||
    (Number.isFinite(maxQty) && qty >= maxQty);
...
const CheckoutUI = () => {
  ...
  // Currency symbol + decimal precision. Priority:
  //   1. checkout (/cart?checkout=1) response — most specific.
  //   2. cart.currency — the active channel's symbol from the cart API (quick vs
  //      ecommerce can differ, so this is channel-wise; see currencyByChannel).
  //   3. global settings — last-resort default.
  // The checkout response often omits `currency`, which blanked the symbol; the
  // channel currency already in redux covers that case.
  const currency =
    checkoutData?.currency || cart?.currency || setting?.setting?.currency;
  const decimals =
    checkoutData?.decimal_point ?? setting?.setting?.decimal_point ?? 0;
  const currencyCode =
    checkoutData?.currency_code ||
    cart?.currency_code ||
    setting?.setting?.currency_code;
  ...
  // ecommerce channel.
  const orderItemIdRef = useRef(null);
  ...
  const fetchAddress = async (opts = {}) => {
    ...
    try {
      ...
      if (response.status == 1) {
        ...
        const defaultAddress = list.find((a) => a.is_default == 1);
  ...
  const handleCommitQty = async (product, variantId, nextQty, ctx) => {
    if (nextQty < 1) return;
    try {
  ...
  const handleRemoveUnits = async (product, variantId, unitsArg, ctx) => {
  ...
  const reapplyCoupon = async (total) => {
  ...
  const handleDeleteAddress = (addr) => setAddressToDelete(addr);
  ...
  const handlePayStackPayment = async (orderId, amount, capitalizedMethod) => {
    try {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (response?.status == 1) {
        ...
        // Capture the ecom order_item_id if the API returned one (top-level,
        // nested, or the first of an order_items array). Quick orders won't have
        // it, so the suffix stays empty and the Quick route is used.
        orderItemIdRef.current =
          response?.order_item_id ??
          response?.data?.order_item_id ??
          response?.data?.order_items?.[0]?.id ??
          response?.data?.order_items?.[0]?.order_item_id ??
          null;
            ...
            <div className="bodyBackgroundColor cardBorder !rounded-lg shadow-sm">
              <div className="flex justify-between items-center gap-3 p-4 md:p-5 border-b">
                <div className="flex items-center gap-2.5 min-w-0 flex-wrap">
                  ...
                  />
                  {/* Location-based ETA + distance beside the title. Quick channel
                      only — the all-shop (ecommerce) channel ships, no local
                      distance/ETA. ShopMode stores all-shop as "allShop". */}
                  {shopMode === "quick" &&
                    !deliveryError &&
                    !checkOutError &&
                    (checkoutData?.time_to_deliver ||
                      checkoutData?.distance) && (
                      <span className="inline-flex items-center gap-2 rounded-lg primaryLightBack px-2.5 py-1.5">
                        {checkoutData?.time_to_deliver && (
                          <span className="flex items-center gap-1.5 text-xs font-semibold primaryColor dark:text-white">
                        ...
                        {checkoutData?.distance && (
                          <span className="flex items-center gap-1.5 text-xs font-medium primaryColor/80 dark:text-white/80">
          ...
          <div className="col-span-12 lg:col-span-4 flex flex-col gap-4 md:gap-6 lg:sticky lg:top-24 lg:self-start">
            ...
            <div className="bodyBackgroundColor cardBorder !rounded-lg shadow-sm overflow-hidden">
              ...
              <div className="px-4 md:px-5 py-4 md:py-5 flex flex-col gap-3">
                ...
                <Row
                  label={`${t("sub_total")} (${itemCount} ${t("items")})`}
                  value={money(
                    checkOutError
                      ? cart?.cartSubTotal
                ...
                {!checkOutError && checkout?.orderType == "doorstep" && (
                  <Row label={t("delivery_charge")} value={money(shipping)} />
                )}
...
const payMethodIcon = (method) => {
  ...
  const logo = payMethodLogos[method];
  if (logo) {
    return (
      <Image
        src={logo}
...
const RecommendationStrip = ({ id, title, subtitle, products }) => {
  const rtl = isRtl();
...
const payMethodSubtitle = (method) => {
  const map = {
...
const PayPickRow = ({ opt, active, onPick }) => {
...
const Row = ({ label, value, onRemove, badge }) => (
