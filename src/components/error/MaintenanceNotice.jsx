...
const MaintenanceNotice = () => {
  ...
  const isUpcoming =
    mode === "0" && Boolean(startTime) && now !== null && startTime > now;
  ...
  const startLabel = dateTimeFmt.format(startTime);
  ...
  const endLabel = endTime
    ? new Date(endTime).toDateString() === new Date(startTime).toDateString()
