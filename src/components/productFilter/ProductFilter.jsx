...
const Filter = ({
  ...
  const [draftBrandIds, setDraftBrandIds] = useState(filter?.brand_ids ?? []);
  const [draftAttrIds, setDraftAttrIds] = useState(
  ...
  useEffect(() => {
    ...
    if (selectedAttrIds.length > 0) {
      (categoryFilters?.attributes ?? []).forEach((a) => {
        const owns = (a?.values ?? []).some((v) =>
  ...
  useEffect(() => {
    const min = categoryFilters?.min_price ?? categoryFilters?.price_min;
    const max = categoryFilters?.max_price ?? categoryFilters?.price_max;
  ...
  const catMin = categoryFilters?.min_price ?? categoryFilters?.price_min;
  const catMax = categoryFilters?.max_price ?? categoryFilters?.price_max;
  ...
  const handleCategoryChange = (categories, slugs) => {
  ...
  const toggleAttributeValue = (valueId) => {
    const id = Number.parseInt(valueId, 10);
