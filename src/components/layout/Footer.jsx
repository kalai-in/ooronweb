...
const Footer = () => {
  ...
  const isLoggedInRaw = useSelector((state) => !!state?.User?.jwtToken);
  ...
  // Shares its query key with Location.jsx's countries fetch.
  const { data: countries = [] } = useQuery({
    queryKey: ["countries", city?.latitude, city?.longitude],
    queryFn: () =>
      api
        .getCountries({
          limit: 100,
  ...
  const present = (val) => isHydrated && val !== "";
