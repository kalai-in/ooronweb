...
const ColumnsIcon = ({ count = 4, size = 18 }) => {
  const gap = 2;
...
// with a saved city.
const Products = ({ initialProducts = null }) => {
  ...
  const persistedListingSource = useSelector(
    (state) => state.ProductFilter.listing_source,
  );
  const persistedCategorySlug = useSelector(
    (state) => state.ProductFilter.category_slug,
  );
  ...
  const filter = useMemo(
    () => ({ ...urlFilter, listing_source, category_slug }),
    [urlFilter, listing_source, category_slug],
  );
  ...
  const resolvedSlugRef = useRef(null);
  useEffect(() => {
    ...
    const leafEntries = childCategories.flatMap((child) =>
  ...
  // Tracks whether category_id changed since the last render, read (not
  // reacted to) at render time by useInfiniteQuery below to decide whether
  // this fetch may keep showing the previous page's products. A ref, not
  // state, because it must be current DURING the render that fires the new
  // query — a state update wouldn't land until the render after.
  const prevCategoryIdRef = useRef(filter?.category_id);
  const categoryChangedRef = useRef(false);
  // Intentional: these refs must be current DURING this render for
  // useInfiniteQuery below (see comment above); a state update wouldn't land
  // until the render after.
  /* eslint-disable react-hooks/refs */
  categoryChangedRef.current =
    prevCategoryIdRef.current !== filter?.category_id;
  prevCategoryIdRef.current = filter?.category_id;
  ...
  const fetchProducts = async ({ pageParam = 0 }) => {
    const filterParams = {
      ...
      // Home-layout "See All": always pass data_source. For "manual" add the id
      // CSV; for "category"/"brand" send block_source_id under that source's own
      // param name (category_id / brand_id). Never block_id.
      ...(filter?.data_source && { data_source: filter.data_source }),
  ...
  const {
    ...
    // Keep the previous page's products on screen while a new filter/sort
    // refetches (new queryKey) — EXCEPT when the category just changed.
    // Without this at all, the grid unmounts to a full skeleton on every
    // attribute/brand/sort toggle (the "glitch" this exists to fix). But
    // carrying it across a CATEGORY change shows the old category's products
    // (wrong count, wrong items) for a moment before the new category's
    // results land — a worse glitch than the skeleton, since a category
    // switch is a full context change, not a minor refinement of the same
    // list. So: keep-previous within a category, full reload across one.
    // eslint-disable-next-line react-hooks/refs -- intentional: ref must be current DURING this render, see comment above
    placeholderData: categoryChangedRef.current ? undefined : keepPreviousData,
  });
  ...
  // legitimately returns empty for this key.
  // Intentional: ref must be current DURING this render, see comment above.
  /* eslint-disable react-hooks/refs */
  const isResolvingCategory =
    ...
    resolvedSlugRef.current !== category_slug;
  /* eslint-enable react-hooks/refs */
  // Latches true the render category_id changes and stays true until the
  // query actually SETTLES (isFetching false), not just until isLoading
  // clears. isResolvingCategory alone left a gap: it drops to false the
  // instant the leaf CSV is dispatched, but that dispatch's new query key
  // hasn't started fetching yet — for one paint, `data` still held the
  // parent id's (empty) result and nothing marked the view as loading, so
  // "No product found" flashed before the leaf-keyed fetch even began.
  // Categories only change loading state — sort/brand/attribute keep
  // keepPreviousData's smoother "old products, no flash" behaviour, so this
  // must NOT key off isFetching alone (that covers every refetch).
  // Intentional: this ref must be current DURING this render (read into
  // `loading` immediately below), not one render later.
  /* eslint-disable react-hooks/refs */
  const categoryLoadingRef = useRef(false);
  if (categoryChangedRef.current || isResolvingCategory) {
    categoryLoadingRef.current = true;
  } else if (!isFetching) {
    categoryLoadingRef.current = false;
  }
  const loading = isLoading || categoryLoadingRef.current;
  /* eslint-enable react-hooks/refs */
  ...
  const handlePrices = async (result) => {
    ...
    const prices = products
      .map((p) => Number(p?.min_price ?? p?.price))
