...
const PaymentStatus = () => {
  ...
  // Currency + precision, matching the order-detail bill (FinalCheckoutSummary):
  // prefer the order's own currency, else settings.
  const currency = order?.currency ?? setting?.currency;
  const decimals = order?.decimal_point ?? setting?.decimal_point ?? 2;
  const money = (v) =>
    v == null || v === "" ? null : formatCurrency(v, currency, decimals);
  const has = (v) => v != null && Number(v) !== 0;
  ...
  const viewOrder = () => {
    ...
    // order_item_id (set at placement) is a fallback when the row didn't load.
