...
import { hasReachedAllowedQty, exceedsAllowedQty } from "@/utils/helperFunction";
...
const ProductDetailModal = ({
  ...
  const handleAddToCart = async () => {
    ...
    if (cart?.isGuest) {
      if (exceedsAllowedQty(productQty + quantity, product?.total_allowed_quantity)) {
  ...
  const currency =
    productDetails?.currency ?? setting?.setting?.currency;
  const decimalPoint =
    productDetails?.decimal_point ?? setting?.setting?.decimal_point ?? 0;
  ...
  // Stepper ceiling: smaller of stock (when limited) and the per-product allowed
  // quantity. Unlimited stock → only the allowed cap applies.
  ...
  const {
    ...
    onRemove: (ctx, units = 1) => serverRemoveFromCart(ctx, units),
