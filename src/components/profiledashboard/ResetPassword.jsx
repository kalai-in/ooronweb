 ...
 *
 * @param {boolean}  embedded   drop the card chrome (border/shadow/rounding) —
 *                              the modal supplies its own container
 * @param {()=>void} onSuccess  called after a successful change (modal closes)
 * @param {()=>void} onCancel   renders a Cancel button beside the CTA
 */
const ResetPassword = ({ embedded = false, onSuccess, onCancel }) => {
    const dir = useDir();
    ...
    const field = ({ id, label, value, onChange, placeholder, show, setShow, hasError, children }) => (
