...
const TransactionCard = ({ transaction }) => {
    ...
    // Settings no longer carry `currency`, so `setting.currency` alone rendered a
    // bare "830" with no symbol. Same priority chain the cart/checkout use:
    // transaction row → active channel's cart currency → global settings.
    const currency =
        transaction?.currency || cart?.currency || setting?.currency || "";
    const decimals =
        transaction?.decimal_point ?? setting?.decimal_point ?? 0;
