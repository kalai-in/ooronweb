...
const BreadCrumb = ({ title, categoryId, categorySlug }) => {
  ...
  useEffect(() => {
    if (router.pathname) {
      ...
      const { rest: pathArray, lang } = parseLangPath(
  ...
  }, [router.pathname, router.asPath, title, parseLangPath]);
  ...
  const handleNotFoundRoutes = (href) => {
    ...
    if (route === "/product") {
      ...
      return router.push({ pathname: zoneHref("/products"), query });
    }
    if (route === "/blog") {
