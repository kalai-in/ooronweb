...
const NewAddressModal = ({
  ...
  const [zoneServiceable, setZoneServiceable] = useState(null);
  const [zoneChecking, setZoneChecking] = useState(false);
  const zoneCheckTimer = useRef(null);
  ...
  const [mobileLengthError, setMobileLengthError] = useState(null);
  const [altMobileLengthError, setAltMobileLengthError] = useState(null);
  ...
  const adoptZoneForAddress = async (lat, lng) => {
    try {
  ...
  const handleConfirmAddress = async (e) => {
    ...
    if (!isAddressSelected) {
      ...
      const prevIds = (addresses?.allAddresses || []).map((a) => a.id);
  ...
  const checkZoneServiceable = (lat, lng) => {
  ...
  const handleMobileChange = ({ dialCode, rawPhone, lengthError }) => {
  ...
  const handleAlternateMobileChange = ({ dialCode, rawPhone, lengthError }) => {
  ...
  const handleMobileCountryReady = (country) => {
  ...
  const handleAltMobileCountryReady = (country) => {
    if (!country?.dial_code) return;
    setaddressDetails((state) =>
